...
def generate_termite_data(input_summary='summary.txt'):
    """
    ...
    with open(topics_file, 'wb') as outfile:
        ...
        with open(input_summary, 'rb') as f:
            ...
            try:
                ...
                sys.exit('file %s, line %d: %s' % (input_summary, reader.line_num, e))
...
class Termite(object):
    ...
    def __init__(self, input_summary='summary.txt'):
        self.topics_file, self.termite_file = generate_termite_data(input_summary)

    def create(self, output_html='termite.html'):
        t = Table(CSV(self.termite_file, columns=['topic', 'word', 'result']))
        df = into(DataFrame, t)

        top = Table(CSV(self.topics_file, columns=['topic', 'word', 'result']))
